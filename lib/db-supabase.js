...
// Mirror לטבלאות יחסיות — אופציונלי, best-effort, לא חוסם שמירה
let mirror = null;
try { mirror = require('./db-mirror'); } catch (_) {}
...
async function saveDB(db) {
  ...
  // שיקוף אסינכרוני לטבלאות יחסיות — לא חוסם את ה-response
  if (mirror && mirror.ENABLED) {
    mirror.mirrorDb(clone(db)).catch(e => console.warn('[mirror]', e.message));
  }
}
